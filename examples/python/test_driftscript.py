...
class TestConnectors:
    ...
    def test_and(self, ds):
        ...
        assert r.value == "<A && B>." or r.value == "(A && B)."
    ...
    def test_ext_image1(self, ds):
        r = ds.compile_one("(believe (inherit (/1, R, X) Y))" if False else "(believe (inherit (ext-image1 R X) Y))")
...
class TestCall:
    ...
    def test_call_no_args(self, ds):
        ...
        # Should be bare ^press, not <... --> ^press>
        assert "<" not in r.value.split("^press")[0].split("&/")[-1] or "^press" in r.value
